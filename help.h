...
void myLine(int x1, int y1, int x2, int y2,int r, int g=0, int b=0) //use three 3 integers if you want colored lines.
{
	HWND console_handle = GetConsoleWindow();
    HDC device_context = GetDC(console_handle);

    //change the color by changing the values in RGB (from 0-255) to get shades of gray. For other colors use 3 integers for colors.
	HPEN pen =CreatePen(PS_SOLID,2,RGB(b,g,r)); //2 is the width of the pen
...
bool isCursorKeyPressed(int& whickKey)   //whichKey passed as reference.... 
{
	...
	key = (char)GetAsyncKeyState(27);
	if (key == 1)
	{

		whickKey = 6;   // 6 if esc key is pressed
		return true;
	}
...
void myRect(int x1, int y1, int x2, int y2, int R, int G, int B)
{
    ...
    SelectObject(device_context,pen);
	HBRUSH brush = ::CreateSolidBrush(RGB(4,200,40)); //Fill color is passed as parameter to the function!!!
